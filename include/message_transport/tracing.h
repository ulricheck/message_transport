...
#define MSGT_TRACE_MESSAGE_PUBLISHED(msgid, topic) \
    if (MESSAGE_TRANSPORT_MESSAGE_PUBLISHED_ENABLED()) { MESSAGE_TRANSPORT_MESSAGE_PUBLISHED(msgid, topic); }

#define MSGT_TRACE_MESSAGE_SERIALIZED(msgid, port) \
    if (MESSAGE_TRANSPORT_MESSAGE_SERIALIZED_ENABLED()) { MESSAGE_TRANSPORT_MESSAGE_SERIALIZED(msgid, port); }

#define MSGT_TRACE_MESSAGE_RECEIVED(port) \
    if (MESSAGE_TRANSPORT_MESSAGE_RECEIVED_ENABLED()) { MESSAGE_TRANSPORT_MESSAGE_RECEIVED(port); }

#define MSGT_TRACE_MESSAGE_DESERIALIZED(msgid, port) \
    if (MESSAGE_TRANSPORT_MESSAGE_DESERIALIZED_ENABLED()) { MESSAGE_TRANSPORT_MESSAGE_DESERIALIZED(msgid, port); }

#define MSGT_TRACE_MESSAGE_CALLBACK(msgid, topic) \
    if (MESSAGE_TRANSPORT_MESSAGE_CALLBACK_ENABLED()) { MESSAGE_TRANSPORT_MESSAGE_CALLBACK(msgid, topic); }
